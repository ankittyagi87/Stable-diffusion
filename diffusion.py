...
class UNetAttentionBlock(nn.Module):
    def __init__(self, n_heads: int, n_embed: int, d_context = 768):
        ...
        self.geglu_linear_1 = nn.Linear(channels, 4 * channels * 2)
        self.geglu_linear_2 = nn.Linear(4 * channels, channels)
    ...
    def forward(self, x, context):
        ...
        x, gate = self.geglu_linear_1(x).chunk(x, dim = -1)

        x = x * F.gelu(gate)

        x = self.geglu_linear_2(x)
